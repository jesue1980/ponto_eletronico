...
def save_foto_auxiliar_data_url(data_url, funcionario_id):
    return save_data_url_file(data_url, funcionario_id, "aux_face", "jpg")
...
def totem_facial_funcionarios_face():
    rows = query(
        """SELECT f.id, f.nome, f.matricula, f.foto_base_path, f.face_embedding,
                  b.id biometria_id, b.embeddings_json, b.foto_principal_path, b.criado_em biometria_criada_em
           FROM funcionarios f
    ...
    if request.method == "POST":
        ...
        execute("""INSERT INTO funcionarios
                   (empresa_id, local_id, jornada_id, nome, cpf, matricula, cargo, email, telefone, data_admissao,
                    ...
                    permitir_totem_facial, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (request.form["empresa_id"], request.form["local_id"], jornada_id, request.form["nome"],
                 ...
                 permite_totem_facial, permite_totem_facial, now_iso()))
...
def funcionarios_api_rows():
    ...
    sql = """SELECT f.id, f.empresa_id, f.local_id, f.jornada_id, f.nome, f.cpf, f.matricula, f.cargo,
                    f.email, f.telefone, f.data_admissao, f.rh_local_id, f.chefia_id, f.secretario_id,
...
def api_funcionario_editar(funcionario_id):
    ...
    if not login:
        return {"erro": "Informe o login do funcionario."}, 400
    ...
    if entrada and saida_final:
        carga = minutes_between(entrada, saida_almoco) + minutes_between(retorno_almoco, saida_final) if saida_almoco and retorno_almoco else minutes_between(entrada, saida_final)
        nome_jornada = f"{request.form.get('matricula', funcionario['matricula'])} - Jornada editada {datetime.now().strftime('%Y%m%d%H%M%S')}"
        execute(
            """INSERT INTO jornadas
               (nome, carga_minutos, entrada, saida_almoco, retorno_almoco, saida_final, tolerancia_minutos, tipo_escala, padrao)
               VALUES (?, ?, ?, ?, ?, ?, 10, 'dias_uteis', 0)""",
    ...
    execute(
        """UPDATE funcionarios
           SET empresa_id = ?, local_id = ?, jornada_id = ?, nome = ?, cpf = ?, matricula = ?,
               ...
               permite_totem_facial = ?, permitir_totem_facial = ?, updated_at = ?
           WHERE id = ?""",
        (
