...
def nav_links(css):
    ...
    if user and user["perfil"] == "RH Local":
        links += [
            ("jornadas", "bi-calendar2-week", "Jornadas"),
        ]
...
def totem_facial_registrar_teste():
    try:
        ...
        if not funcionario_id:
            app.logger.warning("Totem facial teste sem funcionario_id")
            return {"erro": "Selecione um funcionario para o teste."}, 400
        if not totem_id:
        ...
        app.logger.info(
            "Totem facial teste registrou marcacao id=%s funcionario_id=%s nsr=%s",
            marcacao["id"],
            funcionario["id"],
            nsr,
        )
